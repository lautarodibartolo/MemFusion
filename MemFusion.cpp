...
namespace PLMD
{
    namespace colvar
    {
        ...
        class memFusion : public Colvar
        {
            std::vector<AtomNumber> UMEM, LMEM, TAILS;
            std::vector<double> NSMEM, DSMEM, HMEM, RCYLMEM, ZETAMEM, ONEOVERS2C2CUTOFF, XCYL, YCYL;

        public:
            explicit memFusion(const ActionOptions &);
            void calculate() override;
            static void registerKeywords(Keywords &keys);
        };

        PLUMED_REGISTER_ACTION(memFusion, "MEMFUSION")

        void memFusion::registerKeywords(Keywords &keys)
        {
            ...
            keys.add("optional", "ZETAMEM", "occupation factor of the membrane.");
        ...
        memFusion::memFusion(const ActionOptions &ao) : PLUMED_COLVAR_INIT(ao)
        {
