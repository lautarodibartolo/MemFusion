        ...
        {           
            ...
            **************************/
            ...
            for(unsigned t = 0; t < nt; t++)
            {
                ...
                // Eq. 21 and 22 Hub & Awasthi JCTC 2017.
                std::vector<double> sx_Mem_loc(NSMEM[0]), sy_Mem_loc(NSMEM[0]), cx_Mem_loc(NSMEM[0]), cy_Mem_loc(NSMEM[0]);

                // Eq. 10 Hub & Awasthi JCTC 2017.
                std::vector<double> Fs_Mem_loc(NSMEM[0]);
                        ...
                        for (unsigned s = s1_Mem[i]; s <= s2_Mem[i]; s++)
                        {
                            ...
                            if (!((x_loc <= -1.0 - HMEM[0]) || (x_loc >= 1.0 + HMEM[0])))
                            {
                                if (((-1.0 + HMEM[0]) <= x_loc) && (x_loc <= (1.0 - HMEM[0])))
                                {
                                    ...
                                    Fs_Mem_loc[s] += 1.0;
                                    sx_Mem_loc[s] += sin(2.0 * M_PI * TailPosition_loc[0]);
                                    sy_Mem_loc[s] += sin(2.0 * M_PI * TailPosition_loc[1]);
                                    cx_Mem_loc[s] += cos(2.0 * M_PI * TailPosition_loc[0]);
                                    cy_Mem_loc[s] += cos(2.0 * M_PI * TailPosition_loc[1]);
                                }
                                else if (((1.0 - HMEM[0]) < x_loc) && (x_loc < (1.0 + HMEM[0])))
                                {
                                    ...
                                    Fs_Mem_loc[s] += aux_loc;
                                    sx_Mem_loc[s] += aux_loc * sin(2.0 * M_PI * TailPosition_loc[0]);
                                    sy_Mem_loc[s] += aux_loc * sin(2.0 * M_PI * TailPosition_loc[1]);
                                    cx_Mem_loc[s] += aux_loc * cos(2.0 * M_PI * TailPosition_loc[0]);
                                    cy_Mem_loc[s] += aux_loc * cos(2.0 * M_PI * TailPosition_loc[1]);
                                }
                                else if (((-1.0 - HMEM[0]) < x_loc) && (x_loc < (-1.0 + HMEM[0])))
                                {
                                    aux_loc = 0.5 + ((3.0 * x_loc + 3.0) / (4.0 * HMEM[0])) - (pow((x_loc + 1.0), 3) / (4.0 * pow(HMEM[0], 3)));
                ...
                {
                    Fs_Mem[s] += Fs_Mem_loc[s];
                    sx_Mem[s] += sx_Mem_loc[s];
                    sy_Mem[s] += sy_Mem_loc[s];
                    cx_Mem[s] += cx_Mem_loc[s];
                    cy_Mem[s] += cy_Mem_loc[s];
                }
            }
            ...
            for(unsigned t = 0; t < nt; t++)
            {
                ...
                // Number of lipid tails within the slice s of the membranes cylinder.
                std::vector<double> Nsp_Mem_loc(NSMEM[0]);

                // Eq. 20 Hub & Awasthi JCTC 2017.
                std::vector<double> Axs_Mem_loc(NSMEM[0]), Ays_Mem_loc(NSMEM[0]);

                // Eq. 29 Hub & Awasthi JCTC 2017.
                double d_ws_Mem_dz_loc;

                // Eq. 8 Hub & Awasthi JCTC 2017.
                double fradial_Mem_loc = 0;
                    ...
                    if (analyzeThisParticle_Mem[i])
                    {
                        ...
                        for (unsigned s = s1_Mem[i]; s <= s2_Mem[i]; s++)
                        {
                            if (Fs_Mem[s] != 0.0)
                            {
                                ...
                                d_ws_Mem_dz_loc = (1 - pow(ws_Mem[s], 2)) * d_faxial_Mem_dz[i + TAILS.size() * s];
                                d_Xsc_Mem_dz_loc += (ws_Mem[s] * d_sx_Mem_dz_loc + d_ws_Mem_dz_loc * (sx_Mem[s] - Xsc_Mem)) / W_Mem;
                                d_Xcc_Mem_dz_loc += (ws_Mem[s] * d_cx_Mem_dz_loc + d_ws_Mem_dz_loc * (cx_Mem[s] - Xcc_Mem)) / W_Mem;
                                d_Ysc_Mem_dz_loc += (ws_Mem[s] * d_sy_Mem_dz_loc + d_ws_Mem_dz_loc * (sy_Mem[s] - Ysc_Mem)) / W_Mem;
                                d_Ycc_Mem_dz_loc += (ws_Mem[s] * d_cy_Mem_dz_loc + d_ws_Mem_dz_loc * (cy_Mem[s] - Ycc_Mem)) / W_Mem;
                            }
                        }
                        ...
                        if (!((x_loc <= -1.0 - HMEM[0]) || (x_loc >= 1.0 + HMEM[0])))
                        {
                            ...
                            {
                                fradial_Mem_loc = 0.5 - ((3.0 * x_loc - 3.0) / (4.0 * HMEM[0])) + (pow((x_loc - 1.0), 3) / (4.0 * pow(HMEM[0], 3)));
                            ...
                            for (unsigned s = s1_Mem[i]; s <= s2_Mem[i]; s++)
                            {
                                Nsp_Mem_loc[s] += fradial_Mem_loc * faxial_Mem[i + TAILS.size() * s];
                                Axs_Mem_loc[s] += faxial_Mem[i + TAILS.size() * s] * d_fradial_Mem_dx[i];
                                Ays_Mem_loc[s] += faxial_Mem[i + TAILS.size() * s] * d_fradial_Mem_dy[i];
                                fradial_Mem_d_faxial_Mem_dz[i + TAILS.size() * s] = fradial_Mem_loc * d_faxial_Mem_dz[i + TAILS.size() * s];
                            }                        
                        }
                    }
                }
                ...
                for (unsigned s = 0; s < NSMEM[0]; s++)
                {
                    Nsp_Mem[s] += Nsp_Mem_loc[s];
                    Axs_Mem[s] += Axs_Mem_loc[s];
                    Ays_Mem[s] += Ays_Mem_loc[s];
                }
            }
